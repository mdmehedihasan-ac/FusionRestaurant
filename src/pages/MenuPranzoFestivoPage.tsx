  ...
  return (
    <main className="page-shell page-shell--inner">
      <div className="menu-pdf-bar">
        <a className="button button--ghost menu-pdf-bar__btn" href="/menu-pranzo-festivo.pdf" download>
          ↓ Scarica il menu PDF
        </a>
      </div>

      <header className="menu-formula-header">
        <p className="section-kicker">Formula Open — Sabato, Domenica e Festivi</p>
        <h1>Pranzo Festivo</h1>
        <div className="menu-formula-header__prices">
          <span className="menu-formula-header__price">Adulti <strong>{pranzoFestivoInfo.formulaPrice}</strong></span>
          <span className="menu-formula-header__price">Bambini {pranzoFestivoInfo.childNote} <strong>{pranzoFestivoInfo.childPrice}</strong></span>
        </div>
        <p className="menu-formula-header__note">{pranzoFestivoInfo.note}</p>
      </header>

      {pranzoFestivoSections.map((section) => (
              ...
              return (
                <article key={item.code} className="menu-item-card">
                  ...
                  <button
                    ...
                    onClick={() => {
                      addItem(cartKey)
                      trackEvent('menu_add_to_cart', { item: cartKey, page: 'pranzo-festivo' })
                    }}
                  >
